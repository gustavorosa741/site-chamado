...
engine = create_engine(f"mysql+pymysql://{usuario}:{senha}@localhost:3306/teste")

Base = declarative_base()
Session = sessionmaker(bind=engine)
session = Session()

class StatusChamado(PythonEnum):
    ABERTO = "Aberto"
...
class Chamado(Base):
    ...
    solucao = Column(Text)
    progresso = Column(Enum(StatusChamado), default=StatusChamado.ABERTO)
    
    # Relacionamentos
    funcionario = relationship("Usuario", back_populates="chamados")
...
class Usuario(Base):
    ...
    # Relacionamento
    chamados = relationship("Chamado", back_populates="funcionario")
